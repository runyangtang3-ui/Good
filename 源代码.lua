...
local function playMusic(id, name)
    ...
    print("正在播放: " .. (name or tostring(id)))
end
...
-- ==================== 搜索ID播放 ====================
local searchIdInput = ""
MusicControl:Textbox({
    Title = "🔢 输入音乐ID",
    Placeholder = "输入ID后点击播放...",
...
MusicControl:Button({
    Title = "▶️ 播放指定ID",
    Description = "播放上方输入的音乐ID",
    Color = Blue,
    Callback = function()
        local id = tonumber(searchIdInput)
        if id then
            playMusic(id, "自定义音乐 " .. tostring(id))
        else
            print("请输入有效的数字ID")
        end
    end,
